...
Como se había planteado en las secciones anteriores, las canciones con mayor cantidad de reproducciones tienen una media de X minutos de duración. Dada una muestra aleatoria n y una población m, planteamos las siguientes hipótesis:

**1. Hipótesis nula:**
Las canciones del género (pop) con mayor cantidad de reproducciones tienen una media menor o igual que la media de todas las canciones con mayor cantidad de reproducciones.


**2. Hipótesis alternativa**
De lo contration, las canciones del género con mayor cantidad de reproducciones (pop) tiene una media mayor que la media de todas las canciones con mayor cantidad de reproducciones.

> Es fundamental recordar que la negación de una hipótesis no implica la acertación de la hipótesis alterna.

Para la primera prueba de hipótesis planteamos los siguientes estadísticos:
- La significancia de la prueba de hipótesis es 5%
- El tamaño de la muestra es n
- La desviación estándar de la muestra es $\sigma$

#### Cálculo de la primera prueba de hipótesis


### Segunda hipótesis

Se sabe que el artista con la mayor cantidad de reproducciones es Ed Sheeran, por lo que de todas sus canciones tiene una media de M minutos de duración. En base a ello, dada una muestra aleatoria n y una poblacion m, planteramos las siguientes hipótesis:

**1. Hipótesis nula:**
Las canciones de Ed Sheeran tienen éxitos (gran cantidad de reproducciones) debido a la duración de sus canciones, por lo que la media entre todas las canciones es menor o igual que X, la media de la cantidad de reproducciones totales (canciones con la mayor cantidad de reproducciones).

**2. Hipótesis alternativa:**
Las canciones de Ed sheeran tiene éxito(gran cantidad de reproducciones) debido a la duración de sus canciones, por lo que la media entre todas las canciones de Ed Sheeran es *mayor* que X (la media de las canciones con la mayor cantidad de reproducciones).

> Es fundamental recordar que la negación de una hipótesis no implica la acertación de la hipótesis alterna.

Para la segunda prueba de hipótesis planteamos los siguientes estadísticos:

- La siginificancia de la prueba de hipótesis es 4%
- El tamaño de la muestra es n
- La desviación estándar de la muestra es $\sigma$
...
**1. Hipótesis nula:**
  ...
  La media de la duración de las canciones explicit es menor o igual que la media del total de canciones del conjunto de datos.

**2. Hipótesis alternativa:**
  
  La media de la duración de las canciones *explicit* es mayor que la media del total de canciones del conjunto de datos.


Para la tercera prueba de hipótesis planteamos los siguientes estadísticos:
  
- La siginificancia de la prueba de hipótesis es 5%
- El tamaño de la muestra es 100
- La desviación estándar de la muestra es 0.81
  
#### Cálculo de la primera prueba de hipótesis
```{r}
alpha <- 0.05
N_explicit_sample <- 100
total_explicit <- R %>% select(Duration_Minutos, Streams) %>% filter(R$Explicit == "TRUE")
explicit_sample <- sample(total_explicit$Duration_Minutos, N_explicit_sample)
mean_explicit_sample <- mean(explicit_sample)
sd_explicit_sample <- sd(explicit_sample)
mean_total_dataset <- mean(R$Duration_Minutos)
zobs <- (mean_total_dataset - mean_explicit_sample)/(sd_explicit_sample/sqrt(N_explicit_sample))
zcrit<-qnorm(alpha,lower.tail=TRUE)
...
De la gráfica podemos ver que el zcalculado se encuentra fuera del zcrítico, por lo que no podemos descartar esta primera hipótesis.
```{r}
pnorm(zobs)
```


El valor de p es mayor que el nivel de significancia, por lo que no hay motivo para rechazar la hipótesis nula.


#### Cálculo de la primera prueba de hipótesis

# Regresión
En esta sección añadiremos un modelo predictivo para cuál podría ser el artista con la mayor cantidad de reproducciones en base al historial de sus últimas predicciones. Para este caso, asumiremos dos tipos de parámetros: valores dependientes e independientes en regresión.
